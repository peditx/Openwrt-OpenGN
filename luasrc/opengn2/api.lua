module("luci.opengn2.api", package.seeall)
...
LOG_FILE = "/tmp/log/opengn2.log"
CACHE_PATH = "/tmp/etc/opengn2_tmp"
...
function set_cache_var(key, val)
	sys.call(string.format('/usr/share/opengn2/app.sh set_cache_var %s "%s"', key, val))
end
function get_cache_var(key)
	local val = sys.exec(string.format('echo -n $(/usr/share/opengn2/app.sh get_cache_var %s)', key))
...
local function get_bin_version_cache(file, cmd)
	sys.call("mkdir -p /tmp/etc/opengn2_tmp")
	if fs.access(file) then
		chmod_755(file)
		local md5 = sys.exec("echo -n $(md5sum " .. file .. " | awk '{print $1}')")
		if fs.access("/tmp/etc/opengn2_tmp/" .. md5) then
			return sys.exec("echo -n $(cat /tmp/etc/opengn2_tmp/%s)" % md5)
		else
			local version = sys.exec(string.format("echo -n $(%s %s)", file, cmd))
			if version and version ~= "" then
				sys.call("echo '" .. version .. "' > " .. "/tmp/etc/opengn2_tmp/" .. md5)
				return version
			end
		end
	end
...
function to_move(app_name,file)
	...
	local flag = sys.call('pgrep -af "opengn2/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/opengn2 stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/opengn2 restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-opengn2 2>/dev/null | awk '{print $3}'")
	if not version or #version == 0 then
		version = sys.exec("apk list luci-app-opengn2 2>/dev/null | awk '/installed/ {print $1}' | cut -d'-' -f4-")
	end
	return version or ""
end

function to_check_self()
	local url = "https://raw.githubusercontent.com/xiaorouji/openwrt-opengn2/main/luci-app-opengn2/Makefile"
	local tmp_file = "/tmp/opengn2_makefile"
	...
	local remote_version = sys.exec("echo -n $(grep 'PKG_VERSION' /tmp/opengn2_makefile|awk -F '=' '{print $2}')")
				.. "-" ..  sys.exec("echo -n $(grep 'PKG_RELEASE' /tmp/opengn2_makefile|awk -F '=' '{print $2}')")
