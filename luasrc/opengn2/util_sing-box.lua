module("luci.opengn2.util_sing-box", package.seeall)
...
function gen_config_server(node)
	...
	if node.outbound_node then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			local outbound_node_t = uci:get_all("opengn2", node.outbound_node)
			...
			outbound = require("luci.opengn2.util_sing-box").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
...
function gen_config(var)
	...
	if dns_listen_port then
		...
		if direct_dns_udp_server then
			local domain = {}
			local nodes_domain_text = sys.exec('uci show opengn2 | grep ".address=" | cut -d "\'" -f 2 | grep "[a-zA-Z]$" | sort -u')
		...
		if api.cacheFileCompareToLogic(CACHE_TEXT_FILE, content) == false then
			...
			if direct_ipset then
				...
				local ipset_prefix_name = "opengn2_" .. node_id .. "_"
			...
			if direct_nftset then
				...
				local family = "inet"
				local table_name = "opengn2"
				local nftset_prefix_name = "opengn2_" .. node_id .. "_"
