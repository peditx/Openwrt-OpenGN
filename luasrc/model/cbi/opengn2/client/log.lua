...
local file_path = '/tmp/opengn2_upload.tar.gz'
local temp_dir = '/tmp/opengn2_bak'
local fd
http.setfilehandler(function(meta, chunk, eof)
	if not fd and meta and meta.name == "ulfile" and chunk then
		...
		sys.call("echo '' > /tmp/log/opengn2.log")
	end
	...
	if eof and fd then
		...
		if fs.access(file_path) then
			...
			if sys.call("tar -xzf " .. file_path .. " -C " .. temp_dir) == 0 then
				...
				sys.call('/etc/init.d/opengn2_server restart > /dev/null 2>&1 &')
			else
