...
module("luci.controller.opengn2", package.seeall)
...
function index()
	if not nixio.fs.access("/etc/config/opengn2") then
		if nixio.fs.access("/usr/share/opengn2/0_default_config") then
			luci.sys.call('cp -f /usr/share/opengn2/0_default_config /etc/config/opengn2')
		else return end
	end
	local api = require "luci.opengn2.api"
	...
	e.dependent = true
	e.acl_depends = { "luci-app-opengn2" }
	--[[ Client ]]
	...
	--[[Components update]]
	entry({"admin", "services", appname, "check_opengn2"}, call("app_check")).leaf = true
	local coms = require "luci.opengn2.com"
	local com
...
function reset_config()
	luci.sys.call('/etc/init.d/opengn2 stop')
	luci.sys.call('[ -f "/usr/share/opengn2/0_default_config" ] && cp -f /usr/share/opengn2/0_default_config /etc/config/opengn2')
...
function link_add_node()
	...
	luci.sys.call("lua /usr/share/opengn2/subscribe.lua add log")
end
...
function get_redir_log()
	...
	local file_path = "/tmp/etc/opengn2/acl/" .. id .. "/" .. name .. ".log"
...
function get_socks_log()
	local name = luci.http.formvalue("name")
	local path = "/tmp/etc/opengn2/SOCKS_" .. name .. ".log"
...
function get_log()
	-- luci.sys.exec("[ -f /tmp/log/opengn2.log ] && sed '1!G;h;$!d' /tmp/log/opengn2.log > /tmp/log/opengn2_show.log")
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/opengn2.log' ] && cat /tmp/log/opengn2.log"))
end

function clear_log()
	luci.sys.call("echo '' > /tmp/log/opengn2.log")
end

function index_status()
	local e = {}
	e["global_status"] = luci.sys.call("/bin/busybox top -bn1 | grep -v 'grep' | grep '/tmp/etc/opengn2/bin/' | grep 'default' | grep 'global' >/dev/null") == 0
	luci.http.prepare_content("application/json")
	luci.http.write_json(e)
end
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/opengn2/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		if use_time:find("%.") then
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/opengn2/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function server_user_log()
	local id = luci.http.formvalue("id")
	if nixio.fs.access("/tmp/etc/opengn2_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/opengn2_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		luci.http.write(content)
	else
		luci.http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/opengn2_server.log' ] && cat /tmp/log/opengn2_server.log"))
end

function server_clear_log()
	luci.sys.call("echo '' > /tmp/log/opengn2_server.log")
end
...
function create_backup()
	local backup_files = {
		"/etc/config/opengn2",
		"/etc/config/opengn2_server",
		"/usr/share/opengn2/domains_excluded"
	}
	local date = os.date("%y%m%d%H%M")
	local tar_file = "/tmp/opengn2-" .. date .. "-backup.tar.gz"
	fs.remove(tar_file)
	local cmd = "tar -czf " .. tar_file .. " " .. table.concat(backup_files, " ")
	api.sys.call(cmd)
	http.header("Content-Disposition", "attachment; filename=opengn2-" .. date .. "-backup.tar.gz")
	http.header("X-Backup-Filename", "opengn2-" .. date .. "-backup.tar.gz")
