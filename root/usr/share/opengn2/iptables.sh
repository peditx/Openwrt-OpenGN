...
IPSET_LOCAL="opengn2_local"
IPSET_LAN="opengn2_lan"
IPSET_VPS="opengn2_vps"

IPSET_LOCAL6="opengn2_local6"
IPSET_LAN6="opengn2_lan6"
IPSET_VPS6="opengn2_vps6"
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		[ "${preloading}" = "1" ] && {
			...
			for shunt_id in $shunt_ids; do
				...
				[ -n "$shunt_node" ] && {
					local ipset_v4="opengn2_${node}_${shunt_id}"
					local ipset_v6="opengn2_${node}_${shunt_id}6"
		...
		[ "${preloading}" = "1" ] && [ -n "$default_node" ] && {
			local ipset_v4="opengn2_${node}_default"
			local ipset_v6="opengn2_${node}_default6"
			ipset -! create $ipset_v4 nethash maxelem 1048576
			ipset -! create $ipset_v6 nethash family inet6 maxelem 1048576
		...
		for sid in $(ls -F ${TMP_ACL_PATH} | grep '/$' | awk -F '/' '{print $1}' | grep -v 'default'); do
			...
			[ "${write_ipset_direct}" = "1" ] && {
				if [ -n "$(get_cache_var "ACL_${sid}_default")" ]; then
					...
					local ipset_white="opengn2_${sid}_white"
					local ipset_white6="opengn2_${sid}_white6"
...
add_firewall_rule() {
	...
	local ipset_global_white="opengn2_global_white"
	local ipset_global_white6="opengn2_global_white6"
...
flush_ipset() {
	...
	for _name in $(ipset list | grep "Name: " | grep "opengn2_" | awk '{print $2}'); do
		destroy_ipset ${_name}
	done
}
