...
LUA_UTIL_PATH=/usr/lib/lua/luci/opengn2
...
lua_api() {
	...
	echo $(lua -e "local api = require 'luci.opengn2.api' print(api.${func})")
}
...
run_xray() {
	...
	[ -n "$dns_listen_port" ] && {
		...
		[ "${write_ipset_direct}" = "1" ] && {
			...
			if [ "${nftflag}" = "1" ]; then
				local direct_nftset="4#inet#opengn2#opengn2_${set_flag}_white,6#inet#opengn2#opengn2_${set_flag}_white6"
			else
				local direct_ipset="opengn2_${set_flag}_white,opengn2_${set_flag}_white6"
			fi
...
run_ipset_chinadns_ng() {
	...
	[ -n "${ipset}" ] && {
		set_names=$ipset
		vps_set_names="opengn2_vps,opengn2_vps6"
	}
	[ -n "${nftset}" ] && {
		set_names=$(echo ${nftset} | awk -F, '{printf "%s,%s", substr($1,3), substr($2,3)}' | sed 's/#/@/g')
		vps_set_names="inet@opengn2@opengn2_vps,inet@opengn2@opengn2_vps6"
	}
	cat <<-EOF > $config_file
		bind-addr 127.0.0.1
...
start() {
	pgrep -f /tmp/etc/opengn2/bin > /dev/null 2>&1 && {
