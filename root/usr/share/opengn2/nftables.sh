...
NFTABLE_NAME="inet opengn2"
NFTSET_LOCAL="opengn2_local"
NFTSET_LAN="opengn2_lan"
NFTSET_VPS="opengn2_vps"

NFTSET_LOCAL6="opengn2_local6"
NFTSET_LAN6="opengn2_lan6"
NFTSET_VPS6="opengn2_vps6"

FORCE_INDEX=0

. /lib/functions/network.sh

FWI=$(uci -q get firewall.opengn2.path 2>/dev/null)
...
gen_nft_tables() {
	if [ -z "$(nft list tables | grep 'inet opengn2')" ]; then
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		[ "${preloading}" = "1" ] && {
			...
			for shunt_id in $shunt_ids; do
				...
				[ -n "$shunt_node" ] && {
					local nftset_v4="opengn2_${node}_${shunt_id}"
					local nftset_v6="opengn2_${node}_${shunt_id}6"
		...
		[ "${preloading}" = "1" ] && [ -n "$default_node" ] && {
			local nftset_v4="opengn2_${node}_default"
			local nftset_v6="opengn2_${node}_default6"
			gen_nftset $nftset_v4 ipv4_addr 0 0
			gen_nftset $nftset_v6 ipv6_addr 0 0
		...
		for sid in $(ls -F ${TMP_ACL_PATH} | grep '/$' | awk -F '/' '{print $1}' | grep -v 'default'); do
			...
			[ "${write_ipset_direct}" = "1" ] && {
				if [ -n "$(get_cache_var "ACL_${sid}_default")" ]; then
					...
					local nftset_white="opengn2_${sid}_white"
					local nftset_white6="opengn2_${sid}_white6"
...
add_firewall_rule() {
	...
	local nftset_global_white="opengn2_global_white"
	local nftset_global_white6="opengn2_global_white6"
...
flush_nftset() {
	...
	for _name in $(nft -a list sets | grep -E "opengn2" | awk -F 'set ' '{print $2}' | awk '{print $1}'); do
...
stop() {
	...
	[ $(config_t_get global flush_set "0") = "1" ] && {
		...
		#flush_table
		flush_nftset
		rm -rf /tmp/etc/opengn2_tmp/singbox*
		rm -f /tmp/etc/opengn2_tmp/geoip-*.json
	}
	flush_include
}
