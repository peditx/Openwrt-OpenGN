...
uci:foreach(appname, "haproxy_config", function(t)
	if t.enabled == "1" then
		...
		if server_node and server_node.address and server_node.port then
			...
			if health_check_type == "opengn_logic" then
...
listen %s
	...
	external-check command "/usr/share/opengn2/haproxy_check.sh"
]], port, port))
	end
	...
	for i, o in ipairs(listens[port]) do
		...
		if o.export ~= "0" then
			sys.call(string.format("/usr/share/opengn2/app.sh add_ip2route %s %s", o.origin_address, o.export))
		end
