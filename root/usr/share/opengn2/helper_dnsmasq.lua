...
function copy_instance(var)
	...
	for line in io.lines("/tmp/etc/dnsmasq.conf." .. DEFAULT_DNSMASQ_CFGID) do
		local filter
		if line:find("opengn2") then filter = true end
	...
	if TMP_DNSMASQ_PATH then
		sys.call("rm -rf " .. TMP_DNSMASQ_PATH .. "/*opengn*")
	end
...
function add_rule(var)
	...
	local cache_text = ""
	local nodes_address_md5 = sys.exec("echo -n $(uci show opengn2 | grep '\\.address') | md5sum")
	...
	local setflag_4= (NFTFLAG == "1") and "4#inet#opengn2#" or ""
	local setflag_6= (NFTFLAG == "1") and "6#inet#opengn2#" or ""
			...
			uci:foreach(appname, "nodes", function(t)
				local function process_address(address)
					...
					if datatypes.hostname(address) then
						...
						set_domain_ipset(address, setflag_4 .. "opengn2_vps," .. setflag_6 .. "opengn2_vps6")
					end
				end
